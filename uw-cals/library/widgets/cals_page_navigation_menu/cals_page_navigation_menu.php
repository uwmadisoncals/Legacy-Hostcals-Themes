<?php //CALS PAGE NAVIGATION WIDGET
/*todo:
- Add widget controls:
	-display on home page
*/

function widget_cals_page_navigation_menu_register(){

	/**
	 *
	 *
	 *
	 * @uses cals_page_navigation_menu()
	 *
	*/
	
	function widget_cals_page_navigation_menu(){ 
	
	global $post;
	
	//run only if not on home page
...
-->		<?php
		...
		//Run the stuff
			...
			if(count($top_ancestor_post)>0){
				//print li.pagenav element, which wraps all elements in hierarchical menus in WP
				echo '<li id="page_nav_menu" class="widget_page_nave_menu">';
	...
	} //EOF widget_cals_page_navigation_menu
	...
	*/
	...
	function cals_page_navigation_menu($post, $parent_before = '', $parent_after= '', $current_page_ancestors, $top_ancestor = '', $depth_count = 0){
		...
		<a href="<?php the_permalink(); ?>" <?php if (is_page($post->ID)){ echo 'class="current_link"';}?> title="<?php the_title();?>"><span class="white"><?php the_title();?></span></a>
	...
	function widget_cals_page_navigation_menu_control() {
		
		//options
			//display on home page/home page link?
...
/*		$options = $newoptions = get_option('widget_akismet');
	...
	//register widget
	wp_register_sidebar_widget('widget_cals_page_navigation_menu', 'CALS Page Navigation Menu', 'widget_cals_page_navigation_menu',	array('description' => 'Displays a full page navigation menu'));
	//wp_register_widget_control('widget_cals_page_navigation_menu', 'CALS Page Navigation Menu', 'widget_cals_page_navigation_menu_control');


} //EOF widget_cals_page_navigation_menu_register



		//Run everything, except on home page (whether it's a static page or default 'Your latests posts' option
		add_action('widgets_init', 'widget_cals_page_navigation_menu_register');
