...
global $weaveriip_slider_opts;
	$weaveriip_slider_opts = array('_number_images',  '_vertical', '_hidetext', '_img_height', '_img_width', '_menu_width',
						 '_text_font', '_borders', '_vert_compress','_note', '_noeffects', '_menu', '_hidemobile', '_hidemenu');
...
function weaveriip_slider_scripts($vers='1') {
	...
	//wp_enqueue_script('weaverMooTools', $url.'includes/pro/slider-js/mootools-core-yc.js',array(),WEAVERII_VERSION);
	//wp_enqueue_script('weaverMooToolsMore', $url.'includes/pro/slider-js/mootools-more-min.js',array(),WEAVERII_VERSION);

	// wp_enqueue_script('weaverBySliderMenu', $url.'includes/pro/slider-js/byslidemenu-min.js',array(),WEAVERII_VERSION);

    // must be at beginning...

	wp_enqueue_script('weaverMooTools', $url.'includes/pro/slider-js/mootools-weaver-combined.min.js',array(),WEAVERII_VERSION,false);
}
...
function weaveriip_slider_output_style($sout) {
	...
	weaverii_f_write($sout,'.weaver-slider a img {display:inline !important;}'); // fix no-slide menu
}
...
function weaveriip_slider_shortcode($args='') {
	...
	if ( weaverii_pro_getopt($sname.'_menu') != '' ) {
		...
		foreach ( (array) $menu_items as $key => $menu_item ) {
			...
			$menu_list .= '<a href="' . $url . '" title="' . $title . '" style="text-decoration:none;" target="' . $target . '">'
				. $menu_text . '<img src="' . $img_url . '" alt="' . $title . '" /></a></li>' . "\n";
